...
class GRFAdapter:
    ...
    Der reine GRF schätzt Treatment-Effekte direkt (Honest Splitting, eigene
    ...
    Dieser Adapter macht GRF transparent für die rubin-Pipeline.
    """
    ...
    def tune(self, X, T, Y):
        """Ruft EconMLs interne GRF-Tune-Methode auf."""
        import numpy as np
        X_np = np.asarray(X, dtype=np.float64)
        T_np = np.asarray(T, dtype=np.float64).ravel()
        Y_np = np.asarray(Y, dtype=np.float64).ravel()
        return self._cf.tune(X_np, T_np, Y_np)
    ...
    reg.register(
        ...
        lambda ctx: GRFAdapter(
            ...
            **{k: v for k, v in ctx.params_for("grf").items()
               if k not in ("n_estimators", "min_samples_leaf", "max_depth")},
        ),
    )
