...
class AnalysisPipeline:
    ...
    def _run_training(self, cfg, X, T, Y, tuned_params_by_model, holdout_data, mlflow):
        ...
        for name in cfg.models.models_to_train:
            # CausalForestDML und GRF können keine fehlenden Werte verarbeiten.
            # Bei fehlenden Werten wird das Modell übersprungen.
            if name in ("CausalForestDML", "GRF") and has_missing:
            ...
            if name == "GRF":
                # GRF forest_fixed_params über "grf" Key in tuned_params setzen
                ctx.tuned_params = dict(ctx.tuned_params or {})
                forest_defaults = dict(getattr(cfg.causal_forest, "forest_fixed_params", {}) or {})
                if forest_defaults:
                    existing = dict(ctx.tuned_params.get("grf") or {})
                    ctx.tuned_params["grf"] = {**forest_defaults, **existing}
                # GRF-Folds laufen immer sequentiell (Deadlock-Prävention) →
            ...
            # GRF: interne EconML-Tune-Methode nutzen (optimiert Wald-Parameter)
            if name == "GRF" and getattr(cfg.causal_forest, "use_econml_tune", False):
                try:
                    tr_idx = _first_crossfit_train_indices(len(X), T, n_splits=cfg.data_processing.cross_validation_splits, seed=cfg.constants.random_seed)
                    X_tune, Y_tune, T_tune = X.iloc[tr_idx], Y[tr_idx], T[tr_idx]
                    tune_max = getattr(cfg.causal_forest, "tune_max_rows", None)
                    ...
                    if tune_max is not None and len(X_tune) > int(tune_max):
                        ...
                        idx = rng.choice(np.arange(len(X_tune)), size=int(tune_max), replace=False)
                        X_tune, Y_tune, T_tune = X_tune.iloc[idx], Y_tune[idx], T_tune[idx]
                    model.tune(X_tune, T_tune, Y_tune)
                    mlflow.log_param("grf__econml_tune", True)
                    self._logger.info("GRF EconML-Tune erfolgreich.")
                except Exception:
                    self._logger.warning("GRF EconML-Tune fehlgeschlagen.", exc_info=True)
                    mlflow.log_param("grf__econml_tune", False)
