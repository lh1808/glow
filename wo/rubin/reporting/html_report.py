...
def generate_html_report(collector: ReportCollector, output_path: str) -> str:
    ...
    if ha:
        ...
        def _assessment_card(data, metrics_list, title_override=None):
            """Rendert eine farbcodierte Assessment-Karte."""
            color, bg, border = data["color"], data["bg"], data["border"]
            label = title_override or data.get("label", "")
            lvl = data.get("level", "none")
            sym = _sym.get(lvl, "●")
            card = f'<div style="margin:14px 28px;padding:18px 24px;background:{bg};border:2px solid {border};border-radius:var(--r)">'
            card += f'<div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">'
            card += f'<span style="font-size:28px;font-weight:700;color:{color}">{sym}</span>'
            card += f'<div style="font-size:18px;font-weight:700;color:{color}">{escape(label)}</div></div>'
            for d in data.get("details", []):
                card += f'<div style="font-size:13px;color:{color};line-height:1.6;padding-left:40px">{escape(d)}</div>'
            if metrics_list:
                card += '<div style="display:flex;gap:24px;margin-top:12px;padding-top:10px;border-top:1px solid ' + border + ';flex-wrap:wrap">'
                for m_lbl, m_val in metrics_list:
                    card += f'<div style="font-size:11px;color:#666"><span style="text-transform:uppercase;letter-spacing:.5px">{m_lbl}</span><br><strong style="font-size:15px;color:{color}">{m_val}</strong></div>'
                card += '</div>'
            card += '</div>'
            return card
