...
const LGBM_LOGO=`data:image/svg+xml,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 48"><path d="M20 2C12 14 6 22 6 32c0 8 6 14 14 14s14-6 14-14C34 22 28 14 20 2z" fill="%2366BB6A"/><path d="M20 12C15 20 11 26 11 33c0 6 4 11 9 11s9-5 9-11C29 26 25 20 20 12z" fill="%2343A047"/><path d="M20 22C17 27 15 31 15 35c0 4 2 7 5 7s5-3 5-7C25 31 23 27 20 22z" fill="%232E7D32"/></svg>')}`;
...
const ADDON_PRESETS = [
  ...
  {key:"grf_tuning",label:"GRF-Tuning",desc:"Grid-Search für CausalForest / CausalForestDML",group:"tuning",
    cfg:{cfTune:true}},
  {key:"all_tuning",label:"Alle Tunings",desc:"BL (50) + FMT (30) + GRF auf einmal",group:"tuning",
    cfg:{tuningEnabled:true,tuningTrials:50,fmtEnabled:true,fmtTrials:30,fmtModels:["NonParamDML","DRLearner"],cfTune:true}},
  // ── Stabilität ──
  {key:"mc_iters",label:"MC-Iterationen",desc:"Cross-Fitting 3× wiederholen – stabilere Residuals",group:"stability",
    cfg:{mcIters:3}},
  // ── Auswertung & Interpretation ──
  {key:"feature_reduction",label:"Feature-Reduktion",desc:"Importance-basierte Feature-Selektion",group:"interpret",
...
const SCENARIO_PRESETS = [
  {key:"sc_schnelltest",label:"Schnelltest",desc:"Minimaler Durchlauf zum Testen der Pipeline. 2 Meta-Learner, kein Tuning.",color:"#57606a",
    template:"bt_quickstart",addons:[]},
  {key:"sc_standard",label:"Standard-Analyse",desc:"Solide Basis. 4 Modelle mit BL-Tuning und SHAP-Analyse.",color:"#9B111E",
    template:"bt_standard",addons:["bl_tuning","explainability"]},
  {key:"sc_tiefenanalyse",label:"Tiefenanalyse",desc:"Maximale Qualität. DML + Forests, alle Tunings, Feature-Selektion, SHAP.",color:"#D4A853",
    template:"bt_dml_focus",addons:["all_tuning","feature_reduction","explainability","ensemble"]},
  {key:"sc_production",label:"Production-Ready",desc:"Vollständige Pipeline für Deployment. Alle Tunings, Bundle, Surrogate, Ensemble.",color:"#1a7f37",
    template:"bt_standard",addons:["all_tuning","bundle","ensemble","explainability"]},
];
...
const PData = ({cfg,set,setCfg,activeBase,setActiveBase,activeAddons,setActiveAddons,view}) => {
  ...
  const applyScenario = (sc) => {
    ...
    const basePre = basePresets.find(p=>p.key===sc.template) || basePresets[0];
    const paths = {x_file:cfg.x_file,t_file:cfg.t_file,y_file:cfg.y_file,s_file:cfg.s_file,eval_x_file:cfg.eval_x_file,eval_t_file:cfg.eval_t_file,eval_y_file:cfg.eval_y_file,eval_s_file:cfg.eval_s_file,hasNaN:cfg.hasNaN,nanCols:cfg.nanCols,seed:cfg.seed,outputDir:cfg.outputDir,savePreds:cfg.savePreds,predsFormat:cfg.predsFormat,histScoreName:cfg.histScoreName,histScoreCol:cfg.histScoreCol,histScoreHigher:cfg.histScoreHigher,expName:cfg.expName,dpRunName:cfg.dpRunName,eval_mask_file:cfg.eval_mask_file};
    // 2. Collect add-on configs
    const addonOverlay = {};
    const addonSet = new Set(sc.addons||[]);
    addonSet.forEach(k => {const a=ADDON_PRESETS.find(x=>x.key===k);if(a)Object.assign(addonOverlay,a.cfg);});
    // 3. Merge
    const merged = {...DEFAULT_CFG,...paths,treatmentType:tt,refGroup:cfg.refGroup||0,selMetric:tt==="multi"?"policy_value":"qini",...basePre.cfg,...addonOverlay};
    if(tt==="multi") merged.models = (merged.models||[]).filter(m=>!btOnly.has(m));
    setCfg(merged);
    setActiveBase(basePre.key);
    setActiveAddons(addonSet);
  };
  ...
  return (
    <>
      ...
      {view==="template"&&<Sec title="Szenario" accent="#6B0D15">
        <Info>Wähle ein Szenario für einen kompletten Startpunkt. Basis-Vorlage + Add-ons werden automatisch gesetzt und können danach angepasst werden.</Info>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:10}}>
          {SCENARIO_PRESETS.map(sc => (
            <button key={sc.key} onClick={()=>applyScenario(sc)}
              style={{padding:"16px 18px",borderRadius:12,border:"1.5px solid #e0d6d7",background:"#faf6f6",cursor:"pointer",textAlign:"left",transition:"all 0.15s"}}
              onMouseEnter={e=>{e.currentTarget.style.borderColor=sc.color;e.currentTarget.style.background="#fff"}}
              onMouseLeave={e=>{e.currentTarget.style.borderColor="#e0d6d7";e.currentTarget.style.background="#faf6f6"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
                <span style={{fontSize:14,fontWeight:700,color:"#24292f"}}>{sc.label}</span>
              </div>
              <div style={{fontSize:11.5,color:"#666",lineHeight:1.5}}>{sc.desc}</div>
            </button>
          ))}
        </div>
      </Sec>}
      ...
      {view==="template"&&<Sec title="3. Add-ons" accent="#D4A853">
        ...
        {[
          {group:"tuning",label:"Tuning"},
          {group:"stability",label:"Stabilität"},
...
const PModels = ({cfg,set,sp,setSp,spFmt,setSpFmt}) => {
  ...
  return (<><Sec title="Kausale Modelle">{mt&&<Info type="warn"><strong>Multi-Treatment:</strong> Nur DML-Modelle verfügbar.</Info>}{cfg.hasNaN&&<Info type="warn"><strong>Fehlende Werte:</strong> CausalForestDML und CausalForest werden automatisch übersprungen (GRF-basierte Modelle können keine fehlenden Werte verarbeiten).</Info>}<div style={{marginBottom:14}}>{[
    ...
    {cfg.explEnabled&&<>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        {[{k:"shap",l:"SHAP",d:"SHapley Additive exPlanations – globale und lokale Erklärungen. Berechnet den exakten Beitrag jedes Features pro Beobachtung."},{k:"permutation",l:"Permutation",d:"Permutation Importance – modell-agnostisch. Misst den Metrik-Verfall bei zufälliger Feature-Permutation. Schneller, aber nur global."}].map(o => {
          ...
          return (
            <button key={o.k} onClick={()=>set({...cfg,explMethod:o.k})} style={{padding:"16px 18px",borderRadius:12,border:active?"2px solid #D4A853":"1.5px solid "+C.border,background:active?"#fffbeb":"#fff",cursor:"pointer",textAlign:"left",transition:"all 0.15s"}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <div style={{width:14,height:14,borderRadius:7,border:active?"4px solid #D4A853":"2px solid #ccc",background:"#fff",flexShrink:0}}/>
                <span style={{fontSize:14,fontWeight:600,color:active?"#7a5a00":"#333"}}>{o.l}</span>
              </div>
              <div style={{fontSize:12,color:C.textMuted,marginTop:8,lineHeight:1.5}}>{o.d}</div>
            </button>
          );
        })}
      </div>
